...
// Ambil pilihan terkait (jika ada)
...
<body>
    ...
    <?php if ($result_pertanyaan): ?>
        <p><strong>Nama Pertanyaan:</strong> <?php echo htmlspecialchars($result_pertanyaan['nama_pertanyaan']); ?></p>
        <p><strong>Tipe Pertanyaan:</strong> <?php echo ucfirst(htmlspecialchars($result_pertanyaan['tipe_pertanyaan'])); ?></p>

        <?php if ($result_pilihan->num_rows > 0): ?>
            <h3>Pilihan:</h3>
            <ul>
                <?php while ($row = $result_pilihan->fetch_assoc()): ?>
                    <li><?php echo htmlspecialchars($row['pilihan']); ?></li>
                <?php endwhile; ?>
            </ul>
        <?php else: ?>
            <p><em>Tidak ada pilihan untuk pertanyaan ini.</em></p>
        <?php endif; ?>
    <?php else: ?>
